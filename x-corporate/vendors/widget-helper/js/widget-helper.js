var x_corporate_upload_frame_obj;
...
( function( $ ) {
	...
	$(document).ready(function($) {
		...
		$(document).on('click', 'input.wh-image-picker', function( event ){
			...
			var xCorporateImageObject = wp.media.controller.Library.extend({
				defaults : _.defaults({
					id: 'x-corporate-state-insert',
			...
			x_corporate_upload_frame_obj = wp.media.frames.x_corporate_upload_frame_obj = wp.media({
				button: {
					text: $( this ).data( 'uploader_button_text' )
				},
				state : 'x-corporate-state-insert',
				states : [
					new xCorporateImageObject()
				],
				multiple: false
			});

			x_corporate_upload_frame_obj.on( 'select', function() {

				var state = x_corporate_upload_frame_obj.state('x-corporate-state-insert');
			...
			x_corporate_upload_frame_obj.open();
		});
